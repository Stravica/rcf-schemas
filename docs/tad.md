...
Master Technical Architecture Document. One per project. Declares its TACs via `componentIds[]` and its ADRs via `architecturalDecisionIds[]`. The schema defines the FULL set of possible sections; what's required for a given project is a tooling decision.

## Canonical `$id`

`https://schemas.stravica.io/rcf/v0.1.0/tad.schema.json`
...
| `componentIds` | array of `tacId`, min 1 | At least one TAC reference. A TAD with no components describes nothing. |
| `architecturalDecisionIds` | array of `adrId`, min 1 | At least one ADR reference. An architecture with no decisions is not an architecture. |
...
- **Component bodies.** Live in [tac.schema.json](./tac.md), referenced via `componentIds[]`.
- **Decision bodies.** Live in [adr.schema.json](./adr.md), referenced via `architecturalDecisionIds[]`.
